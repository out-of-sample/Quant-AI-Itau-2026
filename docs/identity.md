...
O símbolo vetorial público está em `assets/brand/seriema.svg`, mas ele **não é a assinatura
completa**. O README abre com `assets/brand/seriema-lockup.png`: ave monocromática, wordmark
SERIEMA em Fraunces, título **DO CANTO À CARTEIRA** e arcos do canto. A composição foi reconstruída
a partir das decisões finais de marca; não é um recorte do relatório.

O sistema completo aparece no relatório final `../report/relatorio-seriema.pdf`. Símbolo,
assinatura do repositório e peça editorial são aplicações do mesmo sistema, não logos concorrentes.
