...
(`docs/history/ideation/candidate-theses.md`).
...
> nenhuma fonte entrou em `docs/methodology/data.md` sem ter sido consultada ao vivo, e por isso o documento
...
`docs/methodology/climate-signal.md` marca item a item o que é **CONFIRMADO** (lido da fonte), o que é
...
`docs/references.md`: identificador plausível não é proveniência; copiar somente da fonte.
...
documentos societários foram preservados em `docs/history/audits/ingestion.md`; testes com fixtures reais
...
`docs/methodology/pipeline.md` e checklist do plano mestre. Cada resultado foi classificado como: dívida
transversal (`docs/history/pending.md`), entrega de fase futura, limitação aceita em R-NNN
...
qualquer citação (regra do `docs/references.md`).
...
- **A IA propôs o SETE-ESTRELO (Plêiades) como calendário de plantio brasileiro e a fonte não
  ...
  conferida. Vale o mesmo princípio de `docs/references.md`: pista não é citação.
...
qualquer configuração manual do GitHub. A revisão pública também tomou o `docs/identity.md`
...
**Segunda revisão após a validação do time**: a primeira engenharia pública ainda priorizava o
reprodutor sobre o leitor. Exibia métricas e instruções técnicas cedo, tratava quinze documentos
antigos como entradas equivalentes, usava o símbolo isolado como se fosse a assinatura final e
recortava o painel do relatório em vez de explorar o espaço próprio da repo. O time identificou o
problema central como **progressão**, não falta de conteúdo.

A IA reconstruiu a arquitetura a partir de três leitores e da ordem de suas perguntas. O README
agora percorre tese → mecanismo → falsificação → teste final → veredicto → aprofundamento; só então
chega à reprodução. O método foi separado do histórico científico, sem apagar pré-registros,
decisões ou auditorias. Um atlas novo publica resultados intermediários, safras, custos,
sensibilidades, leave-one-out, atribuição, placebos e múltiplas tentativas. Doze SVGs foram gerados
diretamente de JSON/CSV versionados, com fonte e alternativa textual, em vez de screenshots do PDF.
A abertura passou a usar a assinatura composta aprovada — ave monocromática, wordmark em Fraunces e
**DO CANTO À CARTEIRA** — distinguindo explicitamente símbolo de lockup.

**Validação adicional**: os artefatos `00`–`12` publicados foram comparados byte a byte com a pasta
selada; uma série compacta reconciliou +16,97% da estratégia e +63,31% do livre de risco em 1.186
sessões; os SVGs foram renderizados e inspecionados; testes novos prendem endpoints, claims,
acessibilidade e a ordem narrativa do README. A revisão visual pegou três defeitos que XML válido
não detectaria — texto cortado, labels sobrepostos e uma conclusão longa demais — e eles foram
