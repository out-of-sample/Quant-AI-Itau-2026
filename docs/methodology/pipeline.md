...
Mapeamento direto do esqueleto genérico de `docs/history/ideation/pipeline-and-portfolio.md`,
...
1. **Aplicar `avail_date`** a cada linha, segundo a regra de latência da fonte
   (`docs/methodology/data.md`). Aqui é onde o lookahead é morto na origem.
...
Especificado em detalhe em `docs/methodology/backtest.md`. Invariantes:
universo dinâmico · execução em D+1 · custos explícitos · benchmark declarado a priori.

### C7 — Robustez

Especificado em `docs/history/robustness-plan.md`. É **seção própria**, não um parágrafo.
...
```
.
├── README.md / README.en.md     narrativa bilíngue
├── report/                      relatório final de cinco páginas
├── results/                     atlas, figuras próprias e artefatos públicos
├── docs/
│   ├── README.md                navegação por pergunta
│   ├── methodology/             estratégia → sinal → dados → backtest → software
│   ├── history/                 pré-registros, decisões e auditorias preservadas
│   ├── identity.md              nome e sistema visual
│   ├── genai.md                 uso de IA generativa
│   ├── references.md            bibliografia e proveniência
│   └── assets/                  aplicações finais da marca
├── src/quantagro/
...
│   └── report/              C8 — tabelas e gráficos científicos
├── tests/                       invariantes e regressões
├── scripts/                     interfaces, guards e geradores públicos
├── data/
...
Ver o andamento por fase em `docs/history/project-roadmap.md` §4 e as decisões em `docs/history/decisions.md`.
