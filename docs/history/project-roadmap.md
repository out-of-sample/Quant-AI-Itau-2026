...
Detalhes em `docs/history/preregistration.md` §2.
...
|---|---|
| **`docs/history/preregistration.md`** | Qual é a hipótese, formalizada. **Quais testes a falsificam** — pré-registrados, com critério de falha declarado antes de rodar |
| **`docs/methodology/data.md`** | Que dados existem, com que latência, e **quais deles reescrevem o passado** (o problema do *vintage*). Tudo testado ao vivo |
| **`docs/methodology/pipeline.md`** | Como o pipeline é organizado em camadas e qual invariante cada uma garante |
| **`docs/methodology/backtest.md`** | Regras de execução, custos, universo dinâmico, disciplina do holdout |
| **`docs/history/robustness-plan.md`** | Todos os testes de robustez, **com o resultado esperado declarado antes** |
| **`docs/history/adversarial-review.md`** | O projeto atacado por um avaliador hostil. Onde a defesa é fraca, está escrito que é fraca |
| **`docs/history/decisions.md`** | Riscos vivos + **log de todas as decisões de desenho, com data** |
| **`docs/identity.md`** | Nome e identidade visual da estratégia |
| **`docs/methodology/climate-signal.md`** | Quando o clima importa, por cultura e estado — janelas e limiares agronômicos |
| **`docs/references.md`** | Referências acadêmicas, métodos e fontes de dados usados, com proveniência e lacunas marcadas |
| **`docs/history/audits/ingestion.md`** | Evidências do fechamento da ingestão: cross-check dos preços, decisões sobre preços de commodities e limite de vintage do ComexStat |
| **`docs/history/pending.md`** | Fonte única das dívidas legadas/transversais que não pertencem a uma fase futura |
| **`docs/history/audits/exposures.md`** | Regra, fontes e registro point-in-time da matriz fundamentalista empresa × cultura |
| **`docs/history/audits/corporate-channels.md`** | Portão econômico entre a matriz PIT e a carteira: efeito-preço, volume próprio, insumo, geografia, hedge e redesenho de H2/H3 |
| **`docs/genai.md`** | Registro contínuo do uso de IA generativa no processo |
...
| `docs/history/ideation/` | As 21 teses avaliadas e por que esta foi escolhida |
...
Resolver as pendências de `docs/methodology/data.md` §7 (ajuste de proventos, calendário CONAB).

Andamento (2026-07-16), toda peça com teste e CI verde (ver `docs/methodology/pipeline.md` §6):
...
> `docs/history/audits/ingestion.md`. C2 `Shock` foi concluído em D-027/D-028; o próximo artefato são
...
> auditoria integral e suas correções estão em D-025 e `docs/history/audits/ingestion.md`.
...
- ✅ método fundamentalista point-in-time pré-registrado antes da classificação (D-032;
  `docs/history/audits/exposures.md`)
...
- ✅ inserido o portão **Fase 3.1 — auditoria dos canais empresariais** antes de score e
  carteira (D-034; `docs/history/audits/corporate-channels.md`)
...
| Uso de IA generativa | 15% | `docs/genai.md` — registro contínuo, com o que a IA acertou **e errou** |
| Conclusão e próximos passos | 10% | Limitações reais em `06`; riscos sem solução declarados como tal em `07` |
| Apresentação do robô | 5% | `docs/identity.md` — nome **SERIEMA** aprovado em 28/07/2026, com o folclore enquadrado como linhagem e não como evidência |
