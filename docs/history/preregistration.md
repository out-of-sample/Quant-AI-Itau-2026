...
> em `docs/history/decisions.md` com data e justificativa. Um plano que muda silenciosamente
...
> log de decisões e no `docs/history/project-roadmap.md`. O holdout de retornos ficou lacrado até 27/07/2026,
...
- `Shock_{c,t}` — índice de estresse climático da cultura `c`, agregado nacionalmente,
  observável em `t` (respeitando o lag de publicação — ver `docs/methodology/data.md`)
...
- **Janela fenológica**: a anomalia só é contada durante a fase crítica da cultura
  ...
  `docs/methodology/climate-signal.md`.
...
`docs/history/audits/exposures.md`: `E = direção × materialidade × peso da cultura`. Os pesos usam
...
1. **O painel de vintages só começa na safra 2017/18** — são ~9 safras. A série histórica
   ...
   limita severamente o poder estatístico desta camada específica (ver `docs/history/robustness-plan.md`
   §6 sobre N efetivo).
...
| Janela fenológica | fixa por cultura × UF em `features/shock_spec.py` | CONAB/ZARC/Embrapa; datas e custo documentados em `docs/methodology/climate-signal.md` |
| Lag de publicação do clima | **7 dias corridos** | Conservador vs. a latência real da fonte (ver `docs/methodology/data.md`); sensibilidade testada em 3/7/14 dias |
...
| Custos | componentes definidos; valores e cenários são gate return-agnóstico da Fase 4.0 | Ver D-054 e `docs/methodology/backtest.md` |
...
| **Viés de sobrevivência do sinal** | testar 21 teses e reportar a que funcionou | As 20 teses descartadas estão documentadas em `docs/history/ideation/candidate-theses.md` com a justificativa da escolha, feita **antes** de qualquer backtest |
