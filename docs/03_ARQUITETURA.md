...
| C6 backtest | `backtest/strategy_spec.py`, `backtest/operational_spec.py`, `backtest/inputs.py`, `backtest/engine.py`, `backtest/diagnostics.py`, `backtest/holdout_spec.py`, `backtest/holdout.py`, scripts | 🟡 | contratos D-053/D-055/D-056; ledger autofinanciado e holdout negado. Gate de aluguel recebe estado auditável, exige negócio recente/completude e testa 1% do estoque contra o patrimônio real pré-ordem, inclusive no custo zero. O motor também exige negociação no close de execução/saída e rejeita retorno total menor que −100%. Fase 4.2 concluída: D-058 destrava o smoke 2018/19 com proxy conservadora de aluguel (R27 mitigado). Smoke de engenharia (D-059): motor são, SMTO3 holdout-only no dev, P&L do dev circular. Diagnósticos 4.3 (D-060): o P&L do dev é aposta de setor, não alpha climático. D-064 congela a decomposição setor×clima; D-067 torna AGRO3×ADTV auditável sem escolha ex post. **D-068 congela ordem, papéis inferenciais, grids, claims e inputs da rodada única; o preflight atesta fontes sem ler parquets e `--execute` continua bloqueado até H4, H5 e executor atômico estarem prontos** |
...
| C7 robustez · C8 report | `backtest/holdout_spec.py`, `scripts/run_holdout_once.py` | 🟡 | pacote H′ de D-068 pré-registrado até o preflight: um confirmatório, H4/H5 como vetos e sensibilidades descritivas. Faltam materializar controles H4, placebo geográfico H5 e implementar a emissão indivisível antes da Fase 6; relatório na Fase 7 |
