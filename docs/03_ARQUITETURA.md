...
│   ├── backtest/            C6 — engine, custos, métricas, diagnostics
...
| C0 eventos | `ingest/events_b3.py`, `ingest/events_statusinvest.py`, `ingest/events_manual.py`, `ingest/events_snapshot.py` | ✅ | B3: dinheiro paginado e ações filtradas por classe/ISIN. StatusInvest: cauda deslistada, nominal via `sov`. Manual: bonificações com fonte primária. Snapshot versionado torna o build offline e registra hash canônico do payload interpretado/horário por fonte (D-057) |
| C0–C1 preço | `prices/adjust.py`, `prices/assemble.py`, `scripts/build_equity_returns.py` | ✅ | retorno total PIT D-014/D-015 dos cinco nomes, agora offline; merge B3×StatusInvest, corte na deslistagem e tripwire que falha em extremo não auditado. Split real 3:1 da SMTO3 validado; extremo real JBSS3 de 22/05/2017 curado com fonte CVM |
| C0 aluguel | `ingest/borrow_b3.py` | 🟡 | fetch/cache/manifesto e parsers reais das tabelas `BTBLoanBalance` e `BTBLendingOpenPosition`; cache só é aceito após reconciliar tabela, data, bytes, SHA-256 e contagem. Preserva taxa repetida sem negócio e linha Total sem dupla contagem. Só o último pregão é servido publicamente; sem série histórica, o custo do short usa proxy conservadora declarada (D-058) |
...
| C2 features | `features/exposure.py` + `data/reference/exposure_fundamental_v1.json` (preço, D-032/D-033) + `data/reference/exposure_hprime_v1.json` (quantidade, D-061) | 🟡 | matriz fundamentalista PIT, sem retornos: valida fonte, datas, direção, materialidade e composição. **v1** = canal de preço/insumo, congelado, ainda consumido pelo teste falsificado de reação de ações (D-042/D-043). **exposure_hprime_v1** = re-derivação return-agnóstica sob H′ de quantidade (D-061): mesma direção/cesta, materialidade AGRO3 1,0→0,50 e JBSS3 0,50→0,25 das fontes já auditadas em D-035, para quebrar o colapso da carteira em dois estados (processadores eram idênticos). É o artefato que a estratégia H′ consome (via `run_smoke_dev`/holdout); o loader e as travas são os mesmos |
...
| C6 backtest | `backtest/strategy_spec.py`, `backtest/operational_spec.py`, `backtest/inputs.py`, `backtest/engine.py`, `backtest/diagnostics.py`, `scripts/run_smoke_dev.py`, `scripts/run_diagnostics_dev.py` | 🟡 | contratos D-053/D-055/D-056; ledger autofinanciado e holdout negado. Gate de aluguel recebe estado auditável, exige negócio recente/completude e testa 1% do estoque contra o patrimônio real pré-ordem, inclusive no custo zero. O motor também exige negociação no close de execução/saída e rejeita retorno total menor que −100%. Fase 4.2 concluída: D-058 destrava o smoke 2018/19 com proxy conservadora de aluguel (R27 mitigado). Smoke de engenharia (D-059): motor são, SMTO3 holdout-only no dev, P&L do dev circular. Diagnósticos 4.3 (D-060, `diagnostics.py`): atribuição por nome (JBSS3 54,6% do P&L bruto) e decomposição setor-vs-clima (incremento de clima ~0, R²=0,84 no spread proteína−produtor) — o P&L do dev é aposta de setor, não alpha climático; descritivo, holdout lacrado. D-064: `sector_orthogonal_decomposition` — decomposição aditiva/exata/return-agnóstica (projeção dos pesos reais nos setoriais ingênuos) pré-registrada para explicar o holdout em setor × resíduo climático, sem mudar a estratégia negociada |
| C7 robustez | `stats/robustness_spec.py` | 🟡 | **pré-registro congelado da suíte de robustez de H1 (D-065)**: grid single-knob (climatologia ±2a, final×prelim, lag +14d, janela ±15d) + placebos espacial/temporal, com critérios executáveis (banda [0,4;2,5], placebo morre <0,5× e p>0,10) e veredito global. Return-agnóstico; runner e números vêm a seguir |
