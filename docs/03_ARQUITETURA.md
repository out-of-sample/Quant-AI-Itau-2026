...
| C1 validação | `validate/pit.py`, `validate/universe.py` | ✅ | carimbo `avail_date` + filtro as-of; `UniverseState` preserva o calendário B3 completo antes da whitelist e expõe traded, seasoning por pregões decorridos, ADTV21, elegibilidade e reason codes. Universo dinâmico validado nas 4 deslistagens reais de 2025. Cross-check dos 19 papéis vivos concluído (`scripts/crosscheck_yahoo.py`, D-025/D-056) |
...
| C6 backtest | `backtest/strategy_spec.py`, `backtest/operational_spec.py`, `backtest/inputs.py`, `backtest/engine.py` | 🔄 | contratos econômico/operacional e contabilidade diária congelados (D-053/D-055/D-056). Adaptadores PIT materializam grãos e cana; agenda gera targets; motor autofinanciado mantém quantidades fixas, reconcilia atribuição/custos/turnover, aplica gates de aluguel inclusive no cenário zero e nega o holdout antes do I/O. Fase 4.1 validada sinteticamente, sem P&L observado. Faltam BDI/SMTO3/estado real de mercado (4.2) e diagnósticos (4.3); R26 restringe o smoke test real do dev a 2018/19 |
