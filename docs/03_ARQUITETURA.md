...
| C2 features | `features/shock_spec.py` | 🟡 | contrato do `Shock` primário congelado e testado (D-023): soja + milho 2ª, UFs/janelas, chuva CHIRPS, mínimo 10 safras e geografia PAM/IBGE→UF→CONAB. PAM + polígonos já existem (D-024); falta a janela fenológica → climatologia expanding → z → agregação UF/nacional |
