...
| C0 geografia | `ingest/pam.py`, `ingest/pam_calendar.py`, `ingest/ibge_geometry.py` | ✅ | PAM/SIDRA 1612 municipal com calendário efetivo 2014–2024, captura datada e pesos *as-of*; inclui soja 2713, milho 2711 e algodão 2689. Malha IBGE 2013 fixa pré-amostra, lida sem GDAL. Cobertura positiva sem polígono falha alto (D-024/D-048) |
| C4 sinal | `signal/convention.py` | 🟡 | a convenção algébrica histórica `S = E·Shock` permanece travada contra inversão acidental, mas foi rejeitada economicamente em D-043. A direção reformulada H′ e a carteira só serão congeladas após concluir os canais de cultura (D-044–D-049) |
...
| C2 features | `features/shock_spec.py` | ✅ | contrato do `Shock` primário congelado e testado (D-023): soja + milho 2ª, UFs/janelas, chuva CHIRPS, mínimo 10 safras e geografia PAM/IBGE→UF→CONAB. `COTTON_WINDOWS` materializa à parte o teste secundário do algodão (D-047), rejeitado em D-049 |
