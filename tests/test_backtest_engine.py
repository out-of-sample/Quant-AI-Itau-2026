...
def _inputs(sessions=None):
    ...
    available = pd.DataFrame(True, index=s, columns=UNIVERSE, dtype=bool)
    return returns, adtv, rates, available
...
def test_close_de_execucao_nao_recebe_retorno_e_bloco_tem_21_intervalos():
    ...
    returns, adtv, rates, available = _inputs(sessions)
    returns.loc[blocks[0].execution_date, "AGRO3"] = 0.50
    returns.loc[sessions[2], "AGRO3"] = 0.10

    result = run_backtest(returns, _schedule(blocks), adtv, rates, available)
...
def test_transicao_conta_close_uma_vez_e_negocia_contra_drift():
    ...
    result = run_backtest(returns, _schedule(blocks, reverse_second=True), adtv, rates, available)
...
def test_indisponibilidade_de_um_short_zera_bloco_inteiro_inclusive_custo_zero():
    ...
    blocks = _blocks(sessions)
    returns, adtv, rates, available = _inputs(sessions)
    available.loc[blocks[0].decision_date, "BRFS3"] = False

    result = run_backtest(returns, _schedule(blocks), adtv, rates, available, scenario="zero")
    assert (result.effective_targets == 0).all().all()
    assert result.block_status.iloc[0]["status"] == "flat_borrow_unavailable"
...
def test_aluguel_incide_21_vezes_sobre_short_antigo_e_nao_no_close_de_entrada():
    ...
    blocks = _blocks(sessions)
    returns, adtv, rates, available = _inputs(sessions)
    result = run_backtest(returns, _schedule(blocks), adtv, rates, available, scenario="base")
...
def test_entrada_e_saida_pagavam_turnover_e_custo_sem_rebalancear_no_meio():
    ...
    blocks = _blocks(sessions)
    returns, adtv, rates, available = _inputs(sessions)
    zero = run_backtest(returns, _schedule(blocks), adtv, rates, available, scenario="zero")
    ...
    base = run_backtest(returns, _schedule(blocks), adtv, rates, available, scenario="base")
...
def test_retorno_ausente_em_posicao_mantida_falha_alto():
    ...
    with pytest.raises(ValueError, match="retorno ausente"):
        run_backtest(returns, _schedule(blocks), adtv, rates, available)
...
def test_holdout_e_bloqueado_antes_de_ler_parquet(monkeypatch, tmp_path: Path):
    ...
    _, adtv, rates, available = _inputs(sessions)
    called = False
    ...
    with pytest.raises(HoldoutLockedError, match="Fase 6"):
        run_backtest_from_parquet(tmp_path / "holdout.parquet", schedule, adtv, rates, available)
    assert not called
